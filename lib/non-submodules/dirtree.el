...
(define-widget 'dirtree-file-widget 'push-button
  ...
  :format         "%[%t%]\n"
  :button-face    'default
  :notify          (lambda (node &rest _)
                     (if (vemv/contains? (widget-get node :tag) ".")
                         (dirtree-select node)
                         (dirtree-expand node) ; XXX catch
                         )))
...
(defun dirtree (root select &optional name)
    ...
    (with-current-buffer buffer
      ...
      (dolist (atree tree-mode-list)
        (if (string= (widget-get atree :file) root)
            (setq tree atree)))
      (or tree
          (setq tree (tree-mode-insert (dirtree-root-widget root (or name root))))))
...
(defun dirtree-in-buffer (root select &optional name)
  "create tree of `root' directory
With prefix arguement select `dirtree-buffer'"
  (interactive "DDirectory: \nP")
  (let ((buffer (get-buffer-create dirtree-buffer))
...
(defun dirtree-root-widget (directory name)
...
;; (define-key dirtree-mode-map "\C-o" 'dirtree-display) ;; vemv
(provide 'dirtree)
